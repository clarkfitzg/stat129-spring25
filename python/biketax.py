...
mission = tree.xpath("/Return/ReturnData/IRS990/ActivityOrMissionDesc/text()")[0]
year = int(tree.xpath("/Return/ReturnData/IRS990/FormationYr/text()")[0])
website = tree.xpath("/Return/ReturnData/IRS990/WebsiteAddressTxt/text()")[0]
...
def findbikes(xmlfile, pattern = r"bicycl| bike "):
    ...
    # If we can't find this pattern, then return None
    if not re.search(pattern, result["mission"], flags=re.IGNORECASE):
        return None

    return result

# Test our function.
findbikes(s990[0], pattern=r'athletics')

# Apply our function to the small sample of files
rn = map(findbikes, s990, r'to')
